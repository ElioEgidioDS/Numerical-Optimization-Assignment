    ...
    def truncated_newton(self, f, gradf, hessf, x0):
        ...
        for k in range(self.kmax): 
            ...
            if not np.isfinite(grad_xk_norm):
                return xk, grad_xk_norm, flag_convergence, "NaN Gradient", k, np.array(xk_sequence)
            ...
            if alpha <= 1e-12: #linesearch failes
                return xk, grad_xk_norm, flag_convergence, "LS", k, np.array(xk_sequence) 
        ...
        return xk, grad_xk_norm, flag_convergence, failure_reason, k, np.array(xk_sequence)