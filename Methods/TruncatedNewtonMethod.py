...
# defines adaptive value of eta_k according tho the order of convergence required
...
# solves the inner linear system Bz = c to find the descent direction pk_{tn} 
    ...
    def inner_CG(self, B, c, z0, etak):
        ...
        failure_reason = "-"
        # Initialization, start with p = 0 (so xk = 0)
        ...
        for j in range(self.jmax):
            ...
            # we stop and return the best direction found so far
            if dBd <= 1e-12:
                if j == 0:
                    # if it happens at the first step the hessian is bad 
                    # return steepest descent c
                    return c, failure_reason
                else:
                    # if it happens after some iterations
    ...
    def truncated_newton(self, f, gradf, hessf, x0):
        ...
        for k in range(self.kmax): 
            ...
            # if p_tn is not a descent direction it's replaced by steepest descent c
            if np.dot(p_tn, grad_xk) >= 0:
                p_tn = c

            # computation of aloha using linesearch
        ...
        # safeguard on max iterations reached -> set failure message
        if failure_CG != "-":
