...
>>>>>>> flag-addition:Methods/ModifiedNewtonMethod.py
        x = x0.copy()
        path = [x.copy()]
        B = 1e-3
        ...
        for i in range(self.max_n):
            ...
            if grad_norm < self.tol: #*max(1,np.linalg.norm(gradient)) #CHECK FOR OTHER STOPPING CRITERIONS
                converge = True
<<<<<<< HEAD:Methods/NewtonMethod.py
                return x, np.array(path), np.linalg.norm(gradient), converge, i, flag
=======
...
<<<<<<< HEAD:Methods/NewtonMethod.py
        return x, np.array(path), np.linalg.norm(gradient), converge, self.max_n, flag
=======
